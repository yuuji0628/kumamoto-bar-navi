...
function googleSearchConfig(env){
  return {
    apiKey:t(env.GOOGLE_CSE_API_KEY,500),
    cx:t(env.GOOGLE_CSE_CX,500)
  };
}
...
async function searchInstagramLeads(env,{area,type,start=1}){
  const cfg=googleSearchConfig(env);
  ...
  u.searchParams.set("start",String(Math.max(1,Number(start)||1)));
  ...
  if(!r.ok){
    return {
      ok:false,
      configured:true,
      error:d?.error?.message||`GOOGLE_CSE_HTTP_${r.status}`,
      detail:d
    };
  }
  ...
  return {
    ...
    query:q,
    leads,
    total:Number(d?.searchInformation?.totalResults||0),
    next_start:d?.queries?.nextPage?.[0]?.startIndex||null
  };
}
  ...
  async fetch(request, env, ctx) {
    ...
    if(url.pathname.startsWith("/api/admin/") && !["/api/admin/login","/api/admin/logout","/api/admin/status"].includes(url.pathname)){
      ...
      if(url.pathname==="/api/admin/leads/search" && request.method==="GET"){
        ...
        if(!result.ok && result.error==="GOOGLE_CSE_NOT_CONFIGURED"){
